 ...
 */
class Token extends \app\models\Token implements IdentityInterface
{
    /**
     * @param $guest
     *
    ...
    {
        $token             = new Token();
        $token->created_at = new Expression('UTC_TIMESTAMP');
        $token->guest_id   = $guest->id;
        $token->hash       = md5(time());
    ...
    {
        return $this->getAuthKey() === $authKey && $this->is_valid = 1;
    }
}