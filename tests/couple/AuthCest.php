...
class AuthCest
{
    ...
    public function signIn(AcceptanceTester $I)
    {
        ...
        $csrfToken = $I->grabAttributeFrom('[name="csrf-token"]', 'content');
        $I->fillField('[name="_csrf"]', $csrfToken);
        ...
        $I->see($code);
        $I->see('Logout');
        //$I->see('Unable to verify your data submission.');
    }

    public function signInWithIncorrectValues (AcceptanceTester $I)
    {
        $I->wantTo('try to sign in with incorrect values');
        $I->amOnPage('couple/default/sign-in');
        $I->see('Admin Login');
        ...
        $I->fillField('Wedding ID', $code);
        $I->click('SIGN IN');
        $I->see('Incorrect wedding code or keyword');
        $I->fillField('Admin Email', $email);
        $I->click('SIGN IN');
        $I->see('Incorrect wedding code or keyword');
        $I->fillField('Admin password', $pass);
        $I->click('SIGN IN');
        $I->see('Incorrect wedding code or keyword');
        $I->click('SIGN IN');
        $I->see('Incorrect wedding code or keyword');
